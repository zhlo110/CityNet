...
{
    /// <summary>
    /// set_task_by_department 的摘要说明
    /// </summary>
    public class set_task_by_department : Security
    {
        protected override void fuctionimp(HttpContext context, System.Collections.IList parameters)
        {
            ...
            if (nCount > 0)
            {
                ...
                //重建
                sql = "select distinct DepartmentID from Department_Task where TaskID=@taskid and State = 2";//全选的Session
                list.Clear();
                list.Add(new DictionaryEntry("@taskid", itaskid));
                DataSet ds = DBAccess.Query(sql, "Department_Task", list);
                if (ds != null)
                {
                    if (ds.Tables.Count > 0)
                    {
                        ...
                        for (i = 0; i < nCount; i++)
                        {
                            ...
                            if (DepartmentID > 0) //将DepartmentID
                            {
                                settaskvisible(DepartmentID, itaskid);
                            }
                        }
                    }
                }
        ...
        private void settaskvisible(int departmentid, int taskid)
        {
            //从[Session_User]中取Userid
            string sql = "select distinct UserID from User_Department where DepartmentID = @did";
            IList list = new ArrayList();
            list.Add(new DictionaryEntry("@did", departmentid));
            DataSet ds = DBAccess.Query(sql, "[User_Department]", list);
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
